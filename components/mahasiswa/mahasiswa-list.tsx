...
export default function MahasiswaList({ mahasiswa, loading, onRefresh }: any) {
  if (loading) {
    return (
        ...
        <table className="w-full">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-sm font-semibold text-slate-900">Angkatan</th>
            ...
            {mahasiswa.length === 0 ? (
              ...
              mahasiswa.map((item: any) => (
                <tr key={item.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-6 py-4 text-sm text-slate-600">{item.angkatan}</td>
